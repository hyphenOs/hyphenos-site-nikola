...
The objective of the effort is not to introduce one to new libraries but instead what can be done just using built-ins and standard library. This post was somewhat inspired by [Dave Beazley's talk built-in Super Heroes](https://www.youtube.com/watch?v=lyDLAutA88s). So let's get started.
...
(These lines are taken from the log file that was used in older version of [Stanford 202 course](https://web.stanford.edu/class/stats202/), however it seems that the internet reference is lost, so I am maintaining this file myself [here](https://github.com/gabhijit/training/blob/master/python-course/weblog.txt).)
